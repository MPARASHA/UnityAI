...
public class LevelOptimizer
{
    ...
    const int MAX_ATTEMPTS = 3000;//The max number of times to iterate the algorithm
    ...
    public Level GetLevel(Evaluator evaluator, float goalValue)
    {
        ...
        while (attempts < MAX_ATTEMPTS && bestFitnessSoFar < 0.9){
            attempts++;
            // Mutate population
            ...
            for (int i = 0; i < NUM_CROSSOVERS; i++){
                Level Sample1 = Sample(population, evaluator, goalValue);
                Level Sample2 = Sample(population, evaluator, goalValue);
                
                Level child = Crossover(Sample1, Sample2);
